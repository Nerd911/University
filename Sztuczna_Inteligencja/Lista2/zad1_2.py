...
def opt_dist2(li, D):
    ...
    D0 = D[0]
    if(len(D) == 1):
        return opt_dist(li, D0)
    s = sum(D[1:])
    l = len(li)
    for i in range(D0,l - s):
        temp = opt_dist(li[:i], D0) + opt_dist2(li[(i+1):], D[1:])
        if temp < result:
            result = temp
    return result
...
class Picture:
    ...
    def _get_dist(self, i, j):
        ...
        # print(1)
        # self.print()
        row_dist = opt_dist2(self.solved[i], self.rows[i])
        col_dist = opt_dist2([row[j] for row in self.solved], self.columns[j])
        # print(self.solved[:][j])
    ...
    def _init_solving(self):
        ...
        self.row_cntr = [opt_dist2(self.solved[i], self.rows[i]) for i in range(self.nr_of_rows)]
        self.column_cntr = [opt_dist2([row[i] for row in self.solved], self.columns[i]) for i in range(self.nr_of_columns)]

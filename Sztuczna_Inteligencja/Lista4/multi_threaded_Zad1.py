...
class Board:
    ...
    def __init__(self, other_board = None):
        ...
        self.fields = copy.deepcopy(other_board.fields)
        self.move_list = copy.deepcopy(other_board.move_list)
    ...
    def decision(self, player, max_p):
        moves = self.moves(player)
        pool = ThreadPool(len(moves))
        f = lambda m: (m, Board(self).do_move(m, player).minmax(1 - player, 0, max_p))
        results = pool.map(f, moves)
        return max(results, key = lambda m : m[1])[0]
    ...
    def minmax(self, player, depth, max_p):
        ...
        moves = self.moves(player)
        pool = ThreadPool(len(moves))
        f = lambda m: Board(self).do_move(m, player).minmax(1 - player, depth+1, max_p)
        values = pool.map(f, moves)
        if player == max_p:
            return max(values)
        else:
            return min(values)

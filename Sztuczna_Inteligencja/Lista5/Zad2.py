...
class Board:
    ...
    def cut_off_test(self, depth, start_time):
        return depth > 30 or timer() - start_time > 0.5
...
class MonteCarlo:
    def __init__(self, board, **kwargs):
        self.board = board
        self.states = []
        self.time = seconds = kwargs.get('time', 0.5)
    ...
    def get_play(self, player):
        ...
        games = 0
        begin = timer()
        while timer() - begin < self.time:
            self.run_simulation(player)
            games += 1

        moves_states = [(p, Board(self.board).do_move(p, player)) for p in legal]
    ...
    def run_simulation(self, player):
        ...
        for t in range(1, self.max_moves + 1):
            legal = board_copy.moves(player)
            moves_states = [(p, Board(board_copy).do_move(p, player)) for p in legal]
            ...
            board_copy.do_move(move, player)
        ...
        for player, state in visited_states:
            ...
            if player == 0 and result < 0 or player == 1 and result > 0:
...
def play():
    ...
    while True:
        ...
        if player == 0:
            mc.board = B
            begin = timer()
            m = mc.get_play(player)
            print("MonteCarlo Time: {}".format(timer()-begin))
        else:
            begin = timer()
            m = B.alpha_beta_decision(player)
            print("AlphaBeta Time: {}".format(timer()-begin))
        B.do_move(m, player)
        player = 1-player
        # input()
        if B.terminal():

...
class Board:
    ...
    def alpha_beta_decision(self, player):
        temp = tuple(self.move_list)
        if temp in opening_boards:
            # print("found opening")
            return opening_boards[temp]
        best_move = None
        alpha = -infinity
        beta = infinity
        value = -infinity
...
def process_data(filename):
    ...
    with open(filename) as f:
        for line in f:
            y, x = line[:-1].split()
            Y.append(int(y))
            X.append([int(c) if c != '_' else None for c in x])
    return (X, Y)

X, y = process_data("bigger.dat")
new_X = []
for x in X:
    board_map = [x[(i*8):(i*8 + 8)] for i in range(8)]
    board = Board()
    board.board = board_map
    new_X.append(board.get_coefficients())

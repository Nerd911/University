...
for runs in range(R):
    ...
    print('Path utility={}'.format(utility))
    U.append(utility)
    show_path(path, (0.5 + 0.5 * random.random(), random.random(), random.random()))

expected_utility = sum(U) / len(U)
    
print('Expected utility {}'.format(expected_utility))
if should_be[task_file] < expected_utility:
    print('RESULT {} [OK]'.format(task_file))
else:
    print('RESULT {} [WARNING] EXPECTED UTILITY SHOULD BE GREATER THAN {}'.format(task_file, should_be[task_file]))
input()    
